  ...
  if (
    ...
    for (const child of node.namedChildren) {
      if (child.type === 'variable_declarator') {
        ...
        if (declName != null) return declName.text;
      }
    }
  }
  ...
  if (node.type === 'decorated_definition') {
    for (const child of node.namedChildren) {
      if (
        ...
        if (innerName != null) return innerName.text;
      }
    }
  }
...
function isFunctionLikeDeclaration(node: TreeSitterNode): boolean {
  for (const child of node.namedChildren) {
    ...
    if (value != null && isFunctionLikeNodeType(value.type)) {
      return true;
    }

    const normalized = child.text.replace(/\s+/g, ' ').trim();
    if (
      /^[^=]+=\s*(?:async\s+)?(?:function\b|\([^)]*\)\s*=>|[A-Za-z_$][\w$]*\s*=>)/.test(
        normalized
      )
    ) {
      return true;
